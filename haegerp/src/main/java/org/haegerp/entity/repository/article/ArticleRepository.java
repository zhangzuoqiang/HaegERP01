...
public interface ArticleRepository extends MyRepository<Article, Long> {
    ...
    @Query(countQuery = "SELECT COUNT(*) "
            + "FROM Article "
            + "WHERE "
            + "((1 = ?1 AND idArticleCategory = ?2) OR 1=?3) "
            + "AND "
            + "( "
                + "(1 = ?4) "
                + "OR "
    ...
    @Query(value = "FROM Article "
            + "WHERE "
            + "( "
                + "(1 = ?1 AND TO_CHAR(ean) LIKE '%' || ?2 || '%') "
                + "OR (1 = ?1 AND UPPER(name) LIKE '%' || UPPER(?2) || '%') "
                + "OR (1 = ?1 AND UPPER(articleCategory.name) LIKE '%' || UPPER(?2) || '%') "
                + "OR (1 = ?1 AND TO_CHAR(priceVat*100, '999.99') LIKE '%' || ?2 || '%') "
                + "OR (1 = ?1 AND TO_CHAR(priceGross, '99999999999999999.99') LIKE '%' || ?2 || '%') "
                + "OR (1 = ?1 AND TO_CHAR(priceSupplier, '99999999999999999.99') LIKE '%' || ?2 || '%') "
                + "OR (1 = ?1 AND TO_CHAR(stock) LIKE '%' || ?2 || '%') "
            + ") "
            + "OR (1 = ?3)")
    /**
     * Diese Methode macht eine Rückfrage zur Datenbank mit den Filtern
     ...
     * @param search Suchen, dass der Benutzer im Feld eingefügt hat.
     * @param enableAll 1 - True; 0 - False.
     * @return Eine Liste mit den Artikeln.
     */
    @Transactional(propagation = Propagation.REQUIRED, readOnly = true)

...
public class Validator {
    ...
    public void validateSalary(String strSalaryFrom, String strSalaryTo) throws ValidatorException {
        if (!strSalaryFrom.equals("") && !strSalaryTo.equals("")) {
            strSalaryFrom = strSalaryFrom.replace(',', '.');
            strSalaryTo = strSalaryTo.replace(',', '.');

            float valueSalaryFrom = 0;
            float valueSalaryTo = 0;
            
            try {
                valueSalaryFrom = Float.parseFloat(strSalaryFrom);
                valueSalaryTo = Float.parseFloat(strSalaryTo);
            } catch (Exception ex) {
                ex.printStackTrace(System.err);
            }
            
            if (valueSalaryFrom > valueSalaryTo)
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Salary From must be smaller than Salary To", "Salary From must be smaller than Salary To"));
        }
    }
}

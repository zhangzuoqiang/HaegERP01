	...
	protected void btnDelete_ActionListener(ActionEvent e) {
		...
		if (row != -1) {
			...
			if (articleNames.length() == 0)
				...
				option = JOptionPane.showConfirmDialog(this, "Delete: " + articleCategory.getName() + "\nAre you sure?\nNote: The following articles are still linked to this Category:\n" + articleNames + "(All products under this Category will be eliminated.\nThis operation is irreversible!)", "", JOptionPane.YES_NO_OPTION);

			if (option == JOptionPane.YES_OPTION) {
				articleController.deleteAllArticleFromCategory(articleCategory);

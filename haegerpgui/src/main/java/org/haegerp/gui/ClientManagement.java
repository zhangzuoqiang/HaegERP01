    ...
    protected void btnDelete_ActionListener(ActionEvent e) {
        ...
        if (row != -1) {
            ...
            if (option == JOptionPane.YES_OPTION) {
                if (clientController.isClientOffersEmpty(client.getIdBusinessPartner())) {
                    clientController.delete(client);
                    loadTable();
                    clientCategoryManagement.loadTable();
                } else {
                    JOptionPane.showMessageDialog(this, "This Client have already offers inserted, therefore cannot be delete.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
        }
    }
